...
To bring work home, close Feather's local file tabs and choose **Return Workspace to This Mac…**. A changed local checkpoint is never overwritten: Feather asks you to preserve and compare both copies manually. On success, the local checkout becomes authoritative, recorded remote tmux sessions end, and the remote checkout stays in place as a retained recovery copy. **Clean Up Remote Copy…** is a separate destructive action; it refuses an unverified marker or newly active recorded session and removes only the exact token-owned checkout after confirmation. Do not manually reuse Feather's `.feather*` ownership paths for unrelated data.
