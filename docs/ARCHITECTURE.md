...
Runtime badges contain only ephemeral shell, running, attention, or exited state. Feather reads one bounded active-pane snapshot for all local sessions every two seconds and for connected remote workspaces every six seconds. The foreground command identifies the current agent instead of trusting the terminal's creation label; known Claude and Codex pane-title activity markers distinguish working from waiting. Feather remembers an acknowledged waiting state until that agent works again. The visible Ghostty surface also forwards progress, command-finished, bell, and desktop-notification actions for immediate updates. This is UI activity detection, not a claim about an agent's semantic reasoning state.
...
10. An explicit return first requires the local fingerprint to equal the original handoff manifest. Feather captures the remote branch, commits, index, working tree, and validated nonignored untracked manifest twice; a fixed recorded base is used even if `origin` advanced after handoff. Credential-like or unsafe paths stop the operation before their contents cross SSH.
11. Before local mutation, unpack and apply the remote payload in an isolated shared clone, regenerate its complete fingerprint, and prove that existing ignored local paths remain ignored and cannot be overwritten. Capture the original local state as a rollback payload, remove only the explicitly captured source-state untracked paths during application or rollback, and fingerprint the local checkout again. A failure after mutation restores and verifies the rollback checkpoint; the remote checkout remains intact throughout.
12. Only after local verification may Feather end the bootstrap and terminal session IDs recorded for that workspace. Persist a returned record that immediately restores local terminal and file authority while retaining the remote ownership record. A separate cleanup action rechecks the central ownership token, checkout-bound token, original handoff manifest, and recorded session state immediately before deleting the exact Feather-created checkout. Running recorded sessions require an explicit end-and-delete confirmation.
13. Keep offline, ownership-mismatched, returned, and locally unlisted records visible from persisted state. Feather performs bounded checks only at startup, selection/attachment events, or explicit reconnect, return, and cleanup actions; it never treats absence from a worktree listing or a failed connection as proof that a remote resource was deleted.
14. Legacy snapshots that stored SSH metadata on one terminal migrate to one worktree record while local-only and clean published handoffs retain their existing behavior. Workspaces created before verified handoff manifests remain reconnectable but cannot be returned or deleted automatically.

This boundary intentionally omits automatic provisioning, continuous or bidirectional synchronization, automatic conflict resolution, implicit remote cleanup, agent forwarding, and provider-session copying.
