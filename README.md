...
- Show ephemeral shell, running, attention, and exited state on terminal and worktree badges. One cancellable tmux `wait-for` client blocks until an app-launched agent completes or a lifecycle/bell event occurs; there is no status polling loop, and state is never written to disk.
- Handoff one clean, published terminal checkpoint to a configured SSH host. Feather verifies the branch exactly matches `origin`, creates a unique remote checkout and private tmux session, prints a handoff summary, then replaces the local attachment only after explicit confirmation. It does not push, forward an SSH agent, copy dirty files, provision infrastructure, or store credentials.
...
To use remote handoff, configure one OpenSSH host alias, port, and absolute remote root in Settings, then test the connection. The host must already have Git, tmux, repository access, and the desired agent CLI authentication. Choose **Terminal → Handoff to Remote…** from a clean local branch after pushing it.
