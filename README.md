...
- Make a clean, published worktree remote-first through a named SSH profile. Feather verifies the branch exactly matches `origin`, creates a unique owned checkout and private tmux workspace, and routes every later ordinary Claude, Codex, and shell process for that worktree to the same remote location. Saved ownership, terminal, and SSH metadata survive app restarts; a dropped SSH attachment never ends or recreates its tmux session. Feather takes one bounded session snapshot at startup, selection, explicit reconnect, or an attachment exit so unreachable hosts stay offline and missing sessions stay visibly exited without a polling loop. Feather does not push, forward an SSH agent, copy dirty files, provision infrastructure, or store credentials.
...
To use a remote workspace, add and select a named SSH profile in Settings, then test the connection. The host must already have Git, tmux, repository access, and the desired agent CLI authentication. Close the worktree's terminals, push its clean named branch, then choose **Terminal → Run Workspace Remotely…**. Feather verifies its ownership marker and saved tmux sessions before attaching after a restart. Use **Reconnect Remote Workspace** for an explicit retry after the host was offline. If a saved session really exited, Feather leaves it visibly exited and never creates a replacement under the old tab.
