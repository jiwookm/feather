...
- Make a clean, published worktree remote-first through a named SSH profile. Feather verifies the branch exactly matches `origin`, creates a unique owned checkout and private tmux workspace, and routes every later Claude, Codex, and shell tab for that worktree to the same remote location. Saved ownership and SSH metadata survive app restarts; unreachable hosts remain offline records and are never inferred deleted. Feather does not poll, push, forward an SSH agent, copy dirty files, provision infrastructure, or store credentials.
...
To use a remote workspace, add and select a named SSH profile in Settings, then test the connection. The host must already have Git, tmux, repository access, and the desired agent CLI authentication. Close the worktree's terminals, push its clean named branch, then choose **Terminal → Run Workspace Remotely…**. Feather verifies its ownership marker before attaching after a restart. Use **Reconnect Remote Workspace** for an explicit retry after the host was offline.
